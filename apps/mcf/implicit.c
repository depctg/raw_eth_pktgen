...
#endif
{
    ...
    cache_token_t tk = cache_request((uint64_t) (new + newpos));
    ...
    while( pos-1 && red_cost > (cost_t)new_pos21->flow )
    {
        ...
        new_pos21 = cache_access_mod_opt_mut(new + pos/2 - 1);
    }
...
#endif
{
    ...
    cache_token_t tk_0 = cache_request((uint64_t) (new));
    ...
    arc_t *new_1 = cache_access_mod_opt(new + 1);
    arc_t *new_2 = cache_access_mod_opt(new + 2);

    cmp = (new_1->flow > new_2->flow) ? 2 : 3;

    arc_t *new_cmp1 = (arc_t *) cache_access_mod_opt_mut(new + cmp - 1);

    while( cmp <= net->max_residual_new_m && red_cost < new_cmp1->flow )
    {
      arc_t *new_pos1 = cache_access_mod_opt_mut(new + pos - 1);
        ...
        if( cmp + 1 <= net->max_residual_new_m ) {
            new_cmp1 = cache_access_mod_opt(new + cmp - 1);
            arc_t *new_cmp = cache_access_mod_opt(new + cmp);
            if( new_cmp1->flow < new_cmp->flow ) 
                cmp++;
        } 

        new_cmp1 = (arc_t *) cache_access_mod_opt_mut(new + cmp - 1);
    }
...
#endif
{
    ...
    arc_t *arcout1 = cache_access_mod_opt(arcout + 1);
    ...
    for (i = 0; i < trips; i ++) {
        if (arcout1->ident != FIXED)
            break;
        arcout += 3;
        arcout1 = cache_access_mod_opt(arcout + 1);
    }
    ...
    for( ; i < trips; i++, arcout += 3)
    {
        ...
        if( arcout1->ident != FIXED )
        {
            arc_t *r_arcout = cache_access_mod_opt_mut(arcout); 
            arc_t *r_fistout = cache_access_mod_opt_mut(r_arcout->head->firstout);

            r_fistout->head->arc_tmp = first_of_sparse_list;
            first_of_sparse_list = arcout + 1;
        }
        arc_t *r_arcout = cache_access_mod_opt(arcout); 
        ...
        arc_t *r_first = cache_access_mod_opt(first_of_sparse_list);
        arcin = r_first->tail->arc_tmp;
        while( arcin )
        {   
            // printf("%p\n", arcin);
            arc_t *r_arcin = cache_access_mod_opt(arcin);
            tail = r_arcin->tail;
            ...
            if( red_cost < 0 )
            {
                ...
                if( new_arcs < net->max_residual_new_m )
                {
                    ...
                    arc_t *r_arcnew = cache_access_mod_opt(arcnew);
    ...
    if( new_arcs )
    {
        ...
        if( resized )
        {
            ...
            {
                arc_t *r_arcnew = cache_access_mod_opt_mut(arcnew);
...
#endif
{
    ...
    else
    {
        ...
        for( susp = 0, arc = new_arc; arc < (arc_t *)stop; arc++ )
        {
            ...
            {
                arc_t *r_new_arc = cache_access_mod_opt_mut(new_arc);
                *r_new_arc = *r_arc;
                new_arc++;
            }
        }
