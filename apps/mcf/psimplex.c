...
#endif
{
    ...
    {   //9365
        ...
        if( (bea = primal_bea_mpp( m, arcs, stop_arcs, &red_cost_of_bea )) )
        {
...
#endif
            // printf("BEA ADDR %p\n", bea);
            arc_t *r_bea = cache_access_mod_opt(bea);
            ...
            if( !iminus )
            {
                arc_t *r_bea = cache_access_mod_opt_mut(bea);
                (*bound_exchanges)++;
            ...
            else 
            {
                ...
                update_tree( !xchange, new_orientation,
                            ...
                            (flow_t)net->feas_tol);
                r_bea = cache_access_mod_opt_mut(bea); 
                r_bea->ident = BASIC; 
                arc_t *r_bla = cache_access_mod_opt_mut(bla); 
                r_bla->ident = new_set;
