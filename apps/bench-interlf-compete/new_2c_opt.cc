...
void visit() {
  ...
  for (int k = 0; k < 4; ++ k) {
    ...
    for (int i = 0; i < n_ahead; ++ i) {
      tags[i] = C2::Op::tag((uint64_t)(arc + (i + mofst) * eles));
    ...
    for (int j = 0; j < n_blocks/2; j++ ) {
      ...
      if (j < n_blocks - n_ahead) {
        ...
        tags[idxn] = C2::Op::tag((uint64_t)(arc + (j+n_ahead+mofst) * eles));
      ...
      arc_t *p = C2::Op::template paddr<arc_t>(offs[idx], (uint64_t)(arc + (j+mofst) * eles));
