...
static std::vector<int> passage_count_vec1;
static std::vector<int> passage_count_vec2;
void print_passage_counts_by_vendor_id(int vendor_id)
{
    ...
    rvector<int> *psg = (rvector<int> *) &get_column<int>("passenger_count");

    rvector<int> *psg_newvec;
    // disagg_alloc
    // -> rvector
    if (vendor_id == 1) {
        passage_count_vec1.reserve(N);
        remotelize(passage_count_vec1, false);
        psg_newvec = (rvector<int> *) &passage_count_vec1;
    } else {
        passage_count_vec2.reserve(N);
        remotelize(passage_count_vec2, false);
        psg_newvec = (rvector<int> *) &passage_count_vec2;
    }
    ...
    *gep_indices = *indices;
    ...
    rvector<int> *gep_vids = (rvector<int> *) (gep_indices + 1);
    *gep_vids = *vids;
    arg_size += sizeof(rvector<int>);

    int *gep_id = (int *) (gep_vids + 1);
    *gep_id = vendor_id;
    arg_size += sizeof(int);

    rvector<int> *gep_psg = (rvector<int> *) (gep_id + 1);
    *gep_psg = *psg;
    arg_size += sizeof(rvector<int>);
    
    rvector<int> *gep_psg_newvec = (rvector<int> *) (gep_psg + 1);
    *gep_psg_newvec = *psg_newvec;
    arg_size += sizeof(rvector<int>);

    size_t s = * (size_t *) call_offloaded_service(2, arg_size, sizeof(size_t));

    // size_t s = passage_count_vec.size();

    rring_init(psgread, int, (2 << 20), 32, (size_t) ((char*)rbuf + (8 << 20)), remoteAddr(psg_newvec->head));

    rring_outer_loop(psgread, int, s) {

        rring_prefetch(psgread, 8);
        rring_inner_preloop(psgread, int);
        rring_sync(psgread);

        rring_inner_loop(psgread, j) {
            int e = _inner_psgread[j];
            if (vendor_id == 1)
                step21_passage_count(e);
            else
                step22_passage_count(e);
        }
    }
