...
int main(int argc, char const *argv[])
{
  ...
  // h->array[0] = new_heap_node(0, 4.2, &node_t);
  // h->array[1] = new_heap_node(1, 3.2, &node_t);
  // h->array[2] = new_heap_node(2, 1.5, &node_t);
  // h->array[3] = new_heap_node(3, 0.3, &node_t);
  // h->array[4] = new_heap_node(4, 10.4, &node_t);
  // h->size = 5;
  ...
  while (!is_heap_empty(h))
  {
    ...
    MinHeapNode *mn_view = access_heap_node_view((uint64_t) mn, 0);
    printf("%d %lf\n", mn_view->v, mn_view->dist);
  }
