...
void visit (std::vector<I>& indices_, std::vector<D>& vec, V1 &visitor1, V2 &visitor2, V3 &visitor3)  {
  ...
  rring_init(rids, uint64_t, access_block_size, 32, (size_t)ring_start_base,            indice_offset);
  rring_init(rvec, uint64_t, access_block_size, 32, (size_t)ring_start_base + (64<<20), vec_offset);
