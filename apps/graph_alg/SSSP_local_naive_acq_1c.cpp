#include <stdio.h>
#include <float.h>
#include "util_disagg_naive_acq_1c.hpp"
#include "common.h"
#include "cache.h"
...
void dijkstra(Graph* graph, int src, double *solution)
{
  ...
  while (!is_heap_empty(heap))
  {
    ...
    for (int i = 0; i < graph->l[t].length; ++ i)
      cache_re_acquire(&graph->l[t].neighbours[i]);
    ...
    for (int i = 0; i < graph->l[t].length; ++ i)
      cache_release(&graph->l[t].neighbours[i],1);
  }
}
...
int main(int argc, char const *argv[])
{
  ...
  printf("CLS: %lu B\n", node_cls);
  printf("CS %f MB\n", node_size / (double)(1<<20));
  node_cache = cache_create(node_size, node_cls, (char *)rbuf);
