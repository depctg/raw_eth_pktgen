...
void add_edge(struct Graph *g, int s, int t, float w)
{
  ...
  __int128_t token = cache_request((uint64_t) &(g->l[s].neighbours[g->l[s].length++]));
...
struct Graph* init_graph(uint8_t redundant, uint8_t need_fake, const char *fpath, int *total_v)
{
  ...
  {
    g->l[i].length = 0;
    g->l[i].neighbours = _disagg_alloc(2, sizeof(GraphNode) * MAX_NS);
  }
