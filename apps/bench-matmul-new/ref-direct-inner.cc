...
const int A_line = 4 * 8;
const int A_total_size = (40 << 20);
...
void _mlir_ciface_main_graph(Tensor_float_2 *C, Tensor_float_2 *A, Tensor_float_2 *B) {
  ...
  for (int64_t m = 0; m < M; ++ m) {
    for (int64_t n = 0; n < N; ++ n) {
      float *lC = CCR::get_mut<float>(pin2(rC, m, n));
      lC[0] = 0;
    }
  }
  ...
  for (int64_t m = 0; m < M; m += 4) {
    for (int64_t n = 0; n < N; n += 8) {
      for (int64_t k = 0; k < K; k += 8) {
        // load C [4x8]
        for (int i = 0; i < 4; ++ i) {
          float *lC = CCR::get<float>(pin2(rC, m+i, n));
          alloca[i] = _mm256_loadu_ps(lC);
        }
        ...
        for (int i = 0; i < 8; i += 4) {
          for (int64_t mm = 0; mm < 4; ++ mm) {
            for (int64_t kk = k + i; kk < k + i + 4; ++kk) {
              float *lA = CCR::get<float>(pin2(A->_aligned_ptr, m+mm, kk));
              ap = _mm256_broadcast_ss(lA);
        ...
        // Store C [4x8]
        for (int i = 0; i < 4; ++ i) {
          float *lC = CCR::get_mut<float>(pin2(rC, m+i, n));
          _mm256_storeu_ps(lC, alloca[i]);
        }
      }
    }
  }
...
int main () {
  ...
  for (int64_t m = 0; m < M; ++ m) {
    for (int64_t n = 0; n < N; ++ n) {
      float *lA = CCR::get_mut<float>(pin2(rA, m, n));
      lA[0] = 0;
    }
  }
