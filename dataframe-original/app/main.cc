...
void print_passage_counts_by_vendor_id(StdDataFrame<uint64_t>& df, int vendor_id)
{
    ...
    // std::cout << "new size: " << passage_count_vec.size() << std::endl;
...
void calculate_trip_duration(StdDataFrame<uint64_t>& df)
{
    ...
    // two solutions:
    //  -> trust the move constructor
    //  -> change to full remotable (implement push back and reserve)
...
int main()
{
    ...
    std::cout << "num indices: " << df.get_index().size() << std::endl;
