...
#if 0
	for (arc_t *ap = net.arcs; ap != net.stop_arcs; ap++) {
		arc_t *a = C1R::get<arc_t>(ap);
            printf( "id %ld: a = (%ld,%ld)\n", 
		    ap - net.arcs, a->tail->number, a->head->number);
	}
#endif
