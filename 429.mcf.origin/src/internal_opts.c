...
static __int128_t prefetch21[2 * DIST];
static __int128_t prefetch1[2 * DIST];
...
#endif
{
    ...
    long postemp = pos;
    // prelouge
    for (int i = 0; pos - 1 && i < DIST; ++ i) {
      prefetch1[i] = cache_request((uint64_t) (new + postemp - 1));
      prefetch21[i] = cache_request((uint64_t) (new + postemp/2 - 1));
      postemp >>= 1;
    }
    ...
    {   
        long ppos = pos / (1 << DIST);
        if (ppos-1) {
          prefetch1[(i + DIST) % (2 * DIST)] = cache_request((uint64_t) (new + ppos - 1));
          prefetch21[(i + DIST) % (2 * DIST)] = cache_request((uint64_t) (new + ppos/2 - 1));
        }

        arc_t *new_pos1 = (arc_t *) cache_access_mut(prefetch1[i % (2 * DIST)]);
        arc_t *new_pos21 = (arc_t *) cache_access_mut(prefetch21[i % (2 * DIST)]);
