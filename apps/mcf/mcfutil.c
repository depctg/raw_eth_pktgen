...
// cache line size 1k
    ...
    uint64_t local_arc = (uint64_t)cache_access_mod_opt_mut(arc);
    ...
    // Steady stage
    for (; arc < stop; arc += 16) {
        arc_t *steady_base = cache_access_mod_opt_mut(arc);
        for (int i = 0; i < 16; ++ i) {
            if (arc + i >= stop)
                return;
...
#endif
{
    ...
    while( node != root )
    {
        while( node )
        {
            arc_t *r_basic_arc = cache_access_mod_opt(node->basic_arc);
...
#endif
{
    ...
    for( node = net->nodes, node++; node != (node_t *)stop; node++ ) {
        arc_t *r_basic_arc = cache_access_mod_opt_mut(node->basic_arc);
